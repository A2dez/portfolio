...
![*Since bottoming out in 2013, after The Crash, the price of houses in Ireland has just kept on rising.*](images/property_prices_since_2010.PNG)

![](images/property_prices_by_county.PNG)

![*Rural counties were slower to become expensive, but in 2025 a house in Westmeath is as expensive as one in Dublin in 2017.*](images/everywhere_is_expensive_now.PNG)
...
property$province <- fct_collapse(property$county, 
                                  "Leinster" = c('Dublin', 'Laois', 'Meath', "Kilkenny", "Carlow", "Kildare", "Wicklow", 
                                                 'Wexford',  'Westmeath', 'Louth', 'Offaly','Longford'),
                                  "Munster" = c("Cork", 'Clare', 'Limerick', 'Tipperary', 'Waterford', 'Kerry'), 
                                  'Connacht-Ulster' = c('Galway','Sligo', 'Mayo', 'Leitrim', 'Roscommon', 'Donegal', 'Monaghan', 'Cavan') ,
)
```
...
(annual_price %>% 
  ...
  scale_x_continuous(breaks = annual_sales$year) + #, position = "top" doeesn't work automatically in ggplotly
    #round max sales up to nearest 10k then add 5k for spacing
  scale_y_continuous(expand = c(0, 0), limits = c(0, max(annual_price$price) + 20000)) +
  labs(title = 'Median price of properties sold in Ireland, 2010-present', 
       y = '', x = '') +
  my_theme
) %>% 
  ggplotly
...
# county_v_year <- 
#   property %>% 
#   filter(price > 8e4 , price < 1e6) %>% 
#   mutate(county = factor(county)) %>% 
#   group_by(province, county, year, quarter, new) %>% 
#   #group_by(county, year %>% factor) %>% 
#   summarise (count = n(),
#              median_price = median(price)) %>% 
#   ungroup %>% 
#   group_by(county) %>% 
#   mutate(price_overall = mean(median_price)) %>% 
#   ungroup %>% 
#   mutate (county = county %>% fct_reorder(price_overall))



county_v_year <- property %>% 
    filter(price > 8e4 , price < 1e6) %>% 
  mutate(county = factor(county)) %>% 
  group_by( county, year, quarter, new) %>% 
  summarise (count = n(), 
             median_price = median(price)) %>% 
  ungroup %>% 
  group_by(county) %>% 
...
plot <- 
  county_v_year %>% 
  ggplot() +
  geom_tile(aes (x = year, #paste(year,quarter),  #paste(year,month), 
              y = county, 
              fill = median_price + seq_along(county)) ) +
  # scale_fill_viridis(option = 'B')
  scale_fill_scico(palette = 'roma', trans = 'log2') +
  labs(title = "Almost everywhere is expensive now") +
